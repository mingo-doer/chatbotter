...
class SequenceToSequence(object):
    ...
    def __init__(self,
                 ...
                 embedding_size=300,#输入词表与输出词表的维度
                 mode='train',#取值为train代表训练，为decode为预训练模式
                 hidden_units=256,#RNN模型中间层大小，encoder和decoder层相同
                 depth=4,#RNN层数
                 beam_width=0,#beamsearch的超参数，用于解码
                 cell_type='lstm',#RNN的神经元类型
                 dropout=0.2,#随机丢弃数据率
                 use_dropout=False,
                 use_residual=False,
                 optimizer='adam', #优化器 adam是随机梯度下降
                 learning_rate=1e-3,  #学习率
                 min_learning_rate=1e-6,  #最小学习率
                 decay_steps=500000, #衰减步数
                 max_gradient_norm=5.0, #梯度正则裁剪的系数
                 max_decode_step=None, #最大decode步数
                 attention_type='Bahdanau', #attention类型
                 bidirectional=False, #是否是双向的encoder
                 time_major=False, #是否使用时间为主要的批量数据
                 seed=0, #层间操作的随机数
                 parallel_iterations=None,#并行执行RNN循环执行的个数，默认无限
                 # 如果为True，那么encoder和decoder就会公用一个embedding
                 share_embedding=False,#是否要使用预训练的embedding
                 pretrained_embedding=False): # 提前训练
        ...
        # 生成均匀分布的初始化器 最小值 ，最大值，随机的种子数 （可以为空），类型
    ...
    def build_single_cell(self, n_hidden, use_residual):
        """ 参数: n_hidden: 隐藏层神经元数量 use_residual:
        是否使用residual wrapper """
